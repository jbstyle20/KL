...
adminip=$( curl -sS https://raw.githubusercontent.com/KL/allow/main/accessmenu | grep $MYIP )
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/KL/allow/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
rm -rf /root/allow
git config --global user.email "kibomamdar@gmail.com"
git config --global user.name "KL"
git clone https://github.com/KL/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/KL/allow.git
git push -f https://${tokengit}@github.com/KL/allow.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/Mjoyvpn/KL/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
...
rm -rf /root/data
rm -rf /root/ipvps.conf
git config --global user.email "kibomandar@gmail.com"
git config --global user.name "KL"
git clone https://github.com/Mjoyvpn/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git commit -m delete
git branch -M main
git remote add origin https://github.com/Mjoyvpn/allow.git
git push -f https://${tokengit}@github.com/Mjoyvpn/allow.git
clear
echo -e "\033[0;34m----------------------------------------\033[0m"
